...
def rundocker(instances=16, samplesperinstance=int(1e5)):
    ...
    for i in range(instances):
        command = " ".join(["nice", "-n", "30", "python3", "01_samplempc.py", "--showplot=False", "--randomseed=None", "--experimentname=Docker_"+str(now)+"_"+str(i)+"_", "--numberofsamples="+str(samplesperinstance)])
        if i==instances-1:
            out = None
        else:
