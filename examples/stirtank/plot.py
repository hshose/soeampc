...
def plot_stirtank(shooting_nodes, u_max, u_min, x_max, x_min, U, X, Ucomp = [], Xcomp = [], latexify=False, plt_show=True):
    ...
    plt.subplot(3, 1, 1)
    line, = plt.step(t, np.append([U[0]], U[:]), label=r'$\Pi_{NN}$', color='b')
    if len(Ucomp) > 0:
        line, = plt.step(t, np.append([Ucomp[0]], Ucomp[:]), label=r'$\Pi_{MPC}$', linestyle='dotted', color='b')
    plt.title('predicted trajectory')
    plt.ylabel('inputs u')
    plt.hlines(u_max, t[0], t[-1], linestyles='dashed', alpha=0.7)
    plt.hlines(u_min, t[0], t[-1], linestyles='dashed', alpha=0.7)
    plt.ylim([-1.2*u_max, 1.2*u_max])
    plt.grid()
    plt.legend(loc=1)

    plt.subplot(3, 1, 2)
    line, = plt.plot(t, X[:, 0], label=r'$\Pi_{NN}$', color='g')
    if len(Xcomp) > 0:
        line, = plt.plot(t, Xcomp[:, 0], label=r'$\Pi_{MPC}$', linestyle='dotted', color='g')
    plt.ylabel('$x_1$')
    plt.hlines(x_max[0], t[0], t[-1], linestyles='dashed', color='green',  alpha=0.7)
    plt.hlines(x_min[0], t[0], t[-1], linestyles='dashed', color='green',  alpha=0.7)
    plt.ylim([1.2*x_min[0], 1.2*x_max[0]])
    plt.grid()
    plt.legend(loc=1)

    plt.subplot(3, 1, 3)
    line, = plt.plot(t, X[:, 1], label=r'$\Pi_{NN}$', color='g')
    if len(Xcomp) > 0:
        line, = plt.plot(t, Xcomp[:, 1], label=r'$\Pi_{MPC}$', linestyle='dotted', color='g')
    plt.ylabel('$x_2$')
    plt.hlines(x_max[1],  t[0], t[-1], linestyles='dashed', color='green',  alpha=0.7)
    plt.hlines(x_min[1], t[0], t[-1], linestyles='dashed', color='green',  alpha=0.7)
    plt.ylim([1.2*x_min[1], 1.2*x_max[1]])
    plt.grid()
    plt.legend(loc=1)
